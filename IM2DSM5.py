...
if fov_x != "NA":
    ...
    if focal != "NA":
        fov1 = np.degrees(2 * np.arctan((sensor_x/2) / focal)), np.degrees(2 * np.arctan((sensor_y/2) / focal))
        fov = fov1[0] * (1 + distortion_a*(sensor_x/sensor_y)**3 + distortion_a*1), fov1[1] * (1 + distortion_a*1**3 + distortion_a*1)
    
    elif focal == "NA":
        ### FOV from GCP
        base_vector = [gcp["X"][0]-cam[0], gcp["Y"][0]-cam[1], gcp["Z"][0]-cam[2]]
        x_fov = []
        y_fov = []
        for nr in range(1,len(gcp)):
            sec_vector = [gcp["X"][nr]-cam[0], gcp["Y"][nr]-cam[1], gcp["Z"][nr]-cam[2]]
        ...
        fov1 = np.degrees(np.nanmean(x_fov)), np.degrees(np.nanmean(y_fov))
        #calculate new fov
        fov = fov1[0] * (1 + distortion_a*(sensor_x/sensor_y)**3 + distortion_a*1), fov1[1] * (1 + distortion_a*1**3 + distortion_a*1)
...
if INS_angle_up == "NA":
    ...
    for i in range(0,len(gcp)):
        ...
        img1 = np.sqrt((np.sqrt((list(gcp["pic_X"])[i]-th_ar.shape[1]/2)**2))**2 + d**2 + (np.sqrt((list(gcp["pic_Y"])[i]-th_ar.shape[0]/2)**2))**2)
        img2 = np.sqrt((np.sqrt((list(gcp["pic_X"])[i-1]-th_ar.shape[1]/2)**2))**2 + d**2 + (np.sqrt((list(gcp["pic_Y"])[i-1]-th_ar.shape[0]/2)**2))**2)
        img3 = np.sqrt((np.sqrt((list(gcp["pic_X"])[i-2]-th_ar.shape[1]/2)**2))**2 + d**2 + (np.sqrt((list(gcp["pic_Y"])[i-2]-th_ar.shape[0]/2)**2))**2)
        
        # calculate real distances in WGS84 to image plane
        gcp1 = [list(gcp["X"])[i]-cam[0],list(gcp["Y"])[i]-cam[1],list(gcp["Z"])[i]-cam[2]] 
        gcp1_new = (gcp1 / np.linalg.norm(gcp1)) * img1
        gcp2 = [list(gcp["X"])[i-1]-cam[0],list(gcp["Y"])[i-1]-cam[1],list(gcp["Z"])[i-1]-cam[2]] 
        gcp2_new = (gcp2 / np.linalg.norm(gcp2)) * img2
        gcp3 = [list(gcp["X"])[i-2]-cam[0],list(gcp["Y"])[i-2]-cam[1],list(gcp["Z"])[i-2]-cam[2]] 
        ...
        angle_left = np.degrees(np.arctan(z[1] / z[0]))
        angles_left.append(angle_left)
        angle_up = np.degrees(np.arccos(np.sqrt(z[0]**2 + z[1]**2) / np.sqrt(z[0]**2 + z[1]**2 + z[2]**2)))
        ...
        img_a = [list(gcp["pic_X"])[i] - th_ar.shape[1]/2, list(gcp["pic_Y"])[i] - th_ar.shape[0]/2]
        img_as.append(img_a)
        ...
        img_ax_angles.append(img_ax_angle)
        ...
        if gcp["pic_Y"][i] > th_ar.shape[0]/2:            
            angle_roll = np.degrees(az_angle - img_ax_angle)
        else:
            angle_roll = np.degrees(img_ax_angle - az_angle)
        angles_roll.append(angle_roll)
    ...
    angle_roll = np.nanmean(angles_roll) + adjust_roll
...
## y Pixelkoordinate from pitch and yaw angle
...
th_ax = np.where(p_x > center_coordinates[0], np.round((r*np.cos(alpha)) + center_coordinates[0]), center_coordinates[0] - np.round(r*np.cos(alpha)))
th_ay = np.where(p_x > center_coordinates[0], np.round((r*np.sin(alpha)) + center_coordinates[1]), center_coordinates[1] - np.round(r*np.sin(alpha)) )
